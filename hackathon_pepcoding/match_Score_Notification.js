...
(async function(){
    try{
        ...
        let matchLinks = await getCorrectMatchLink(page);

        // console.log(matchLinks);
         
        await page.goto(matchLinks)
        setInterval(async()=>{
            await page.reload
...
function sleep (duration){
  ...
  async function getScoreOnInterval(page){
    ...
    return sendScoreCard(scorecard)
    sleep(5000);
 ...
 async function getCorrectMatchLink(page){
      return await page.evaluate(()=>{
        ...
        for(let i=0; i<elements.length; i++){
           let title = document.querySelectorAll("h3 a")[i].innerText;
 ...
 function sendScoreCard(scorecard){
